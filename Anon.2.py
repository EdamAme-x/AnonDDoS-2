...
def choice_down_proxies():
	...
	if choice4=='y'or choice4=='Y':
		...
		if choice_mode=='proxy':
			if sel_pr==_A:urlproxy='https://www.proxy-list.download/api/v1/get?type=http'
			else:urlproxy='https://api.proxyscrape.com/?request=displayproxies&proxytype=http&timeout=5000&country=all&ssl=yes&anonymity=all'
		elif sel_pr==_A:urlproxy='https://www.proxy-list.download/api/v1/get?type=socks5'
		else:urlproxy='https://api.proxyscrape.com/?request=displayproxies&proxytype=socks5&timeout=5000&country=all&ssl=yes&anonymity=all'
		proxyget()
